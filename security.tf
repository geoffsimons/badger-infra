...
# -----------------------------------------------------------------------------
# Backend (Spring Boot/ECS Fargate) Security Group - API
# -----------------------------------------------------------------------------
resource "aws_security_group" "backend" {
  ...
  # This single rule covers ECR/ECS control plane access via the NAT Gateway.
  egress {
...
# -----------------------------------------------------------------------------
# Frontend (React/Nginx/ECS Fargate) Security Group
# -----------------------------------------------------------------------------
resource "aws_security_group" "frontend" {
  name        = "${var.app_name}-Frontend-SG"
  description = "Allows traffic from ALB to the React frontend containers."
  vpc_id      = aws_vpc.app_vpc.id
...
# -----------------------------------------------------------------------------
# Ingress Rules for ECS Tasks (Backend & Frontend)
# -----------------------------------------------------------------------------
# Backend Ingress Rule
resource "aws_security_group_rule" "backend_ingress" {
  description              = "Allow traffic from the ALB to Backend (8080)"
  type                     = "ingress"
  from_port                = 8080
  to_port                  = 8080
  protocol                 = "tcp"
...
resource "aws_security_group_rule" "frontend_ingress" {
  description              = "Allow traffic from the ALB to Frontend (80)"
  type                     = "ingress"
  from_port                = 80 # Assuming your Nginx/Frontend app runs on 80
  to_port                  = 80
  protocol                 = "tcp"
...
output "frontend_security_group_id" {
  description = "The ID of the React Frontend security group."
